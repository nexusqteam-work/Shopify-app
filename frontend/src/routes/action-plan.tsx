...
import { fetchIssues, mapIssues, markIssueFixed, markIssueOpen } from "@/lib/store-data";
...
function ActionPlanPage() {
  ...
  const fixMutation = useMutation({
    mutationFn: ({ issueId, isFixed }: { issueId: string; isFixed: boolean }) =>
      isFixed ? markIssueOpen(issueId) : markIssueFixed(issueId),
    onSuccess: async () => {
      await Promise.all([
  ...
  const sourceIssues = issuesData?.issues ?? [];
  const issues = useMemo(() => mapIssues(sourceIssues), [sourceIssues]);

  const filtered = useMemo(() => {
    const list = issues.filter((issue) => {
      const source = sourceIssues.find((item) => item.id === issue.id);
      const isFixed = !!source?.isFixed;
    ...
    return sorted;
  }, [filter, sort, issues, sourceIssues]);

  const fixedCount = sourceIssues.filter((issue) => issue.isFixed).length;
  const remaining = sourceIssues.length - fixedCount;
  const totalMonthlyLoss = sourceIssues.filter((issue) => !issue.isFixed).reduce((sum, issue) => sum + issue.impact, 0);
  const remainingFixHours = sourceIssues.filter((issue) => !issue.isFixed).reduce((sum, issue) => sum + issue.effortMinutes, 0) / 60;
    ...
    <div className="mx-auto w-full max-w-[1440px] 2xl:max-w-[1720px] px-4 sm:px-6 lg:px-10 xl:px-14 py-6 lg:py-8 xl:py-10">
      <div className="mb-6 animate-fade-up">
        ...
        <p className="text-[14px] mt-1" style={{ color: "var(--text-secondary)" }}>
          Prioritized fixes ranked by monthly revenue impact for {user?.shopName ?? "your store"}.
        </p>
      </div>
              ...
              <div className="p-5">
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1">
                        ...
                        <div className="flex flex-wrap items-center gap-2 mt-2">
                          <span
                            ...
                            style={{
                              background: `color-mix(in oklab, ${priorityColor[issue.priority]} 14%, white)`,
                      ...
                      <div className="text-right shrink-0">
                        ...
                        <div className="display text-[20px] font-bold mono" style={{ color: "var(--danger)" }}>
                          {formatINRFull(issue.revenueImpact)}
                        </div>
                ...
                {isOpen && (
                  ...
                  >
                    ...
                    {source?.shopifyAdminUrl && (
                      <a
                        href={source.shopifyAdminUrl}
                        target="_blank"
                        rel="noreferrer"
