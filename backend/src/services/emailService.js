...
export async function sendEmail({ to, subject, html, text }) {
  try {
    ...
    const info = await transport.sendMail({
      from: process.env.EMAIL_FROM || 'no-reply@Flovix.ai',
      to,
      subject,
      html,
...
export async function sendWelcomeEmail(merchant) {
  await sendEmail({
    to: merchant.email,
    subject: `Welcome to Flovix, ${merchant.shopName}! 🚀`,
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 24px;">
        <div style="background: #0D1320; padding: 32px; border-radius: 12px; text-align: center; margin-bottom: 24px;">
          <h1 style="color: #00C896; margin: 0 0 8px; font-size: 28px;">Welcome to Flovix! 🎉</h1>
        ...
        <p style="color: #5A6A8A; font-size: 15px; line-height: 1.6;">
          Hi there! Flovix is now connected to <strong>${merchant.shopName}</strong>.
        </p>
        <p style="color: #5A6A8A; font-size: 15px; line-height: 1.6;">
